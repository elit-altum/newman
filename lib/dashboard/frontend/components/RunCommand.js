...
const RunCommand = (props) => {
    ...
    const sendPauseCommand = () => {
        if(!isEnded) {
            socket.emit("onPauseProcess", { processId: props.processId });
        }
    }

    const sendAbortCommand = () => {
        if(!isEnded) {
            socket.emit("onAbortProcess", { processId: props.processId });
        }
    };

    const sendResumeCommand = () => {
        if (!isEnded) {
            socket.emit("onResumeProcess", { processId: props.processId });
        }
    };
