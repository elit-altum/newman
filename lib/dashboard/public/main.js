...
const getLocalStorageData = () => {
    const prevRuns = localStorage.getItem("prevRuns");

    if(prevRuns) {
        execRuns.innerHTML = execRuns.innerHTML + JSON.parse(prevRuns);
    }
};

getLocalStorageData();

const setLocalStorage = (currRuns) => {
    const stringCurrRuns = JSON.stringify(currRuns);
    localStorage.setItem('prevRuns', stringCurrRuns);
}
...
socket.on("newProcessFrontend", (data) => {
    ...
    setLocalStorage(execRuns.innerHTML);
});
